createApp({
  ...
  methods: {
      ...
      fetch(this.api + 'concepto/' + id, {
        ...
        if (json.success) {
          if (this.cotizacion.conceptos.has(json.data._id)) {
            ...
            this.cotizacion.conceptos.set(json.data._id, {concepto:{...json.data}, cantidad: 1, total: json.data.precio});
          }
        } else {
          console.error(json.message);
        }
    ...
    update() {
      ...
      const send = {
        ...
        entrega: cotizacion.entrega.dias + ',' + cotizacion.entrega.entr
      };
      ...
      fetch(this.api + 'cotizacion/' + cotizacion.id, {
        'method': 'PUT',
        'headers': {
  ...
  mounted() {
    ...
    fetch(this.api + 'cotizacion/' + this.params.id)
      ...
      .then( (json) => {
        if (json.success) {
          const entrega = json.data._entrega.split(',');
          json.data.entrega = {
            dias: entrega[0],
            entr: entrega[1],
          }
          const conceptos = json.data.conceptos;
          json.data.conceptos = new Map();
          if (json.data.retencionIVA == 1) {
            json.data.retencionIVA = true;
          } else {
            json.data.retencionIVA = false;
          }
          conceptos.forEach( concepto => {
            json.data.conceptos.set(concepto.concepto._id, concepto);
          });
